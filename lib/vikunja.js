(() => {
  ...
  async function listTasks({ projectId = null, filter = '', sortBy = 'id', orderBy = 'desc' } = {}) {
    const { baseUrl, token } = await getConfig();
    if (!baseUrl || !token) {
      throw new Error('Vikunja is not configured yet.');
    }
    ...
    try {
      return await paginate('tasks', { token, baseUrl, baseParams });
    } catch (e) {
      // Older Vikunja instances exposed the all-tasks collection as /tasks/all,
      // which newer versions no longer register (it matches /tasks/:projecttask).
      if (e.status !== 404) {
        throw e;
      }
      return paginate('tasks/all', { token, baseUrl, baseParams });
    }
  }
  ...
  function nearestHours(date = new Date()) {
    const hours = date.getHours();
    if (hours <= 9 || hours > 21) return 9;
  ...
  function dueTodayISO(now = new Date()) {
    ...
    date.setHours(nearestHours(date), 0, 0, 0);
