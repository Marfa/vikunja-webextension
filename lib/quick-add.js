...
(() => {
  ...
  function analyzeTaskText(text, prefixesMode = PrefixMode.Default, now = new Date()) {
    ...
    const consume = (sp, addToken) => {
      const span = origSpan(sp);
      if (span === null) return;
      removed.push(span);
      addToken(span);
    };
    ...
    if (repeatSpan !== null) {
      consume(repeatSpan, ([start, end]) => {
        result.repeats = { text: text.slice(start, end).trim(), start, end };
      });
    }
