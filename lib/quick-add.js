...
(() => {
  ...
  // Map a parsed repeat ({amount, type}) to the task fields the Vikunja API
  // accepts: repeat_after is an interval in seconds (repeat_mode 0), while
  // "every month" uses repeat_mode 1 (monthly). Returns null when there is
  // nothing to set.
  function repeatTaskFields(repeats) {
    if (!repeats || typeof repeats.amount !== 'number' || repeats.amount < 1) {
      return null;
    }
    const secondsPerUnit = {
      [REPEAT_TYPES.Seconds]: 1,
      [REPEAT_TYPES.Minutes]: 60,
      [REPEAT_TYPES.Hours]: 3600,
      [REPEAT_TYPES.Days]: 86400,
      [REPEAT_TYPES.Weeks]: 604800,
      [REPEAT_TYPES.Years]: 31536000,
    };
    if (repeats.type === REPEAT_TYPES.Months) {
      // v2 has no "every N months" mode; monthly it is.
      return { repeat_after: 0, repeat_mode: TASK_REPEAT_MODES.REPEAT_MODE_MONTH };
    }
    const unitSeconds = secondsPerUnit[repeats.type];
    if (typeof unitSeconds !== 'number') {
      return null;
    }
    return {
      repeat_after: repeats.amount * unitSeconds,
