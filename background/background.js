(() => {
  ...
  if (typeof importScripts === 'function') {
    importScripts('../lib/i18n.js', '../lib/vikunja.js');
  }
  ...
  async function reinjectIntoOpenTabs(patterns) {
    ...
    for (const tab of tabs) {
      ...
      await api.scripting.executeScript({ target, files: ['lib/i18n.js', 'content/element.js'] }).catch(() => {});
  ...
  async function syncContentScripts() {
    ...
    const script = {
      ...
      matches: patterns,
      js: ['lib/i18n.js', 'content/element.js'],
  ...
  async function handleCreateTask(message, sender) {
    try {
      ...
      if (!origin || !patterns.includes(`${origin}/*`)) {
        throw new Error(t('errRequestOrigin', 'Request did not come from a registered Element instance.'));
      }
      const title = String(message.title || '').replace(/\s+/g, ' ').trim();
      if (!title) {
        throw new Error(t('errNoMessageText', 'No message text to add.'));
      }
  ...
  async function handleMatrixReact(message, sender) {
    try {
      ...
      if (!api.scripting || typeof api.scripting.executeScript !== 'function') {
        throw new Error(t('errScriptingUnavailable', 'Scripting API unavailable.'));
      }
      const tabId = sender && sender.tab && sender.tab.id;
      if (typeof tabId !== 'number') {
        throw new Error(t('errNoTabToReact', 'No tab to react in.'));
      }
      const roomId = String(message.roomId || '').trim();
      const eventId = String(message.eventId || '').trim();
      const key = String(message.emoji || '📝');
      if (!eventId) {
        throw new Error(t('errMissingEventId', 'Missing event id.'));
      }
  ...
  async function handleMatrixRoomName(message, sender) {
    try {
      ...
      const tabId = sender && sender.tab && sender.tab.id;
      if (typeof tabId !== 'number') {
        throw new Error(t('errNoTabToRead', 'No tab to read from.'));
      }
      const roomId = String(message.roomId || '').trim();
      if (!roomId) {
        throw new Error(t('errMissingRoomId', 'Missing room id.'));
      }
